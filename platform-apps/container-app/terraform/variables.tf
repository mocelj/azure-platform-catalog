...
variable "private_dns_zone_resource_id" {
  type        = string
  description = "Existing region-specific privatelink.swedencentral.azurecontainerapps.io zone."
  nullable    = false
  validation {
    ...
    error_message = "Use the swedencentral Container Apps private-link DNS zone."
  }
}

variable "infrastructure_subnet_resource_id" {
  type        = string
  description = "Separate dedicated Microsoft.App/environments-delegated subnet with NAT egress."
  nullable    = false
  validation {
    condition = (
    ...
    error_message = "The environment must use its dedicated infrastructure subnet, not the PE subnet."
  }
}
