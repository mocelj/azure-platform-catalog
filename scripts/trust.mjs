...
export function validateDispatch(input) {
  if (input.repository !== 'mocelj/azure-platform-catalog' || input.ref !== 'refs/heads/main' || input.actor !== 'mocelj') {
    throw new Error('Connected execution is restricted to the catalog owner dispatching its protected main branch.');
  }
...
export async function fetchConsumer(sha, target, token, fetcher = fetch) {
  ...
  if (!['ahead', 'identical'].includes(comparison.status)) throw new Error('Consumer commit is not an ancestor of approved main; fork/unmerged commits cannot enter the private runner.');
...
export function summarizeTerraformPlan(plan) {
  ...
  for (const resource of plan.resource_changes ?? []) {
    ...
    if (actions.includes('delete')) throw new Error('Destruction/replacement is not permitted by this demo workflow. Use a separately reviewed operator procedure.');
...
export function assertPlanBinding(plan, expected, now = Date.now()) {
  ...
  for (const key of ['target', 'consumerSha', 'catalogSha', 'configHash', 'environmentHash', 'toolchainHash']) {
    if (plan[key] !== expected[key]) throw new Error(`Saved plan binding changed (${key}); create and approve a new plan.`);
  }
...
export function assertModuleGraph(actual, expected) {
  ...
  if (hash(sort(actual)) !== hash(sort(expected))) throw new Error('Terraform module graph drifted from the reviewed release; do not upgrade during deployment.');
}
